...
TEST_CASE("server -- loads `/`", "[weight=2][part=3][first]") {
  time_t t;
...
TEST_CASE("server -- loads `/340.png`", "[weight=3][part=3][second]") {
  time_t t;
  srand((unsigned) time(&t));
  int port = 10000 + (rand() % 40000);
  char cmd[100];
...
TEST_CASE("server -- loads `/getaway.html`", "[weight=3][part=3][third]") {
  time_t t;
  srand((unsigned) time(&t));
  int port = 10000 + (rand() % 40000);
  char cmd[100];
...
TEST_CASE("server -- fails `/not-found.html`", "[weight=2][part=3][fourth]") {
  time_t t;
  srand((unsigned) time(&t));
  int port = 10000 + (rand() % 40000);
  char cmd[100];
